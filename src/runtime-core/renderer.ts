...
export function createRenderer(options: RendererOptions) {
	...
	function patchKeyedChildren(
		...
		if (i > e1) {
			...
			let patched = 0;
			const keyToNewIndexMap = new Map<string, VNode>(); //key to vnode
			//* 将新的节点放入map
			for (let j = i; j <= e2; j++) {
				const nextChild = c2[j];
				keyToNewIndexMap.set(nextChild.key, nextChild);
			}

			for (let j = i; j <= e1; j++) {
				const prevChild = c1[j];
				let nextChild: VNode = null;
				...
				if (prevChild.key) {
					nextChild = keyToNewIndexMap.get(prevChild.key);
				} else {
					for (let k = i; k <= e2; k++) {
